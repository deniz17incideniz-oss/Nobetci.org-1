...
### İstanbul nöbetçi eczane veri stratejisi

- Şimdilik İstanbul için canlı nöbetçi eczane verisi çekilmiyor.
- Kullanıcı İstanbul Eczacı Odası ve e-Devlet TİTCK resmî kaynaklarına yönlendiriliyor.
- API veya veri kullanım izni gelirse bağlantı `fetchIstanbulPharmacies()` adapter fonksiyonu üzerinden yapılacak.
- İstanbul Eczacı Odası’ndan API/veri kullanım izni alınmadan otomatik veri çekilmeyecek ve izinsiz scraping yapılmayacak.

### Büyükşehir nöbetçi eczane yönlendirme stratejisi

- 30 büyükşehir için `/[city]/nobetci-eczane` rehber sayfaları hazırlanır.
- Canlı veri, API veya scraping kullanılmaz; kullanıcı resmî kaynaklara yönlendirilir.
- Kaynağı kesin doğrulanmış şehirlerde şehir eczacı odası bağlantısı kullanılabilir.
- Kaynak doğrulaması bekleyen şehirlerde güvenli fallback olarak e-Devlet TİTCK nöbetçi eczane sorgulama sayfası kullanılır.
- Kaynak listesi `src/data/officialSources.ts` dosyasında tutulur.
